...
const ShotItem: React.FC<{ 
    ...
    const handleEnhancePrompt = async () => {
        ...
        setIsEnhancing(true);
        try {
            const enhancedPrompt = await enhanceVideoPrompt(shot.prompt);
    ...
    return (
        <div className="bg-[var(--bg-tertiary)] p-0 rounded-xl border border-[var(--border-color)] flex flex-col relative group transition-all hover:border-[var(--accent-cyan)] animate-fade-in overflow-hidden shadow-sm hover:shadow-md">
             {/* Header Bar */}
             <div className="flex justify-between items-center bg-[var(--bg-interactive)] px-3 py-2 border-b border-white/5">
                 <div className="flex items-center gap-2">
                     <span className="bg-[var(--accent-blue)] text-white text-xs font-bold px-2 py-0.5 rounded shadow-sm">
                        #{index + 1}
                     </span>
                     {shot.imagePreview && (
                 ...
                 <div className="flex items-center gap-2">
                    <span className={`text-[10px] font-bold uppercase tracking-wider ${
                         shot.status === 'done' ? 'text-green-400' :
                         shot.status === 'processing' ? 'text-blue-400 animate-pulse' :
                         shot.status === 'error' ? 'text-red-400' : 'text-[var(--text-muted)]'
                     }`}>
                        {t(`motionStudio.status.${shot.status}`)}
                     </span>
                     <button 
             ...
             <div className="flex flex-col p-3 gap-2">
                 ...
                 <div className="flex gap-3">
                     {shot.imagePreview && (
                         <div className="w-20 h-20 flex-shrink-0 rounded-lg overflow-hidden border border-white/10 bg-black/40 shadow-inner relative group/img">
                 ...
                 {/* Camera Movement Quick Tools (Horizontal Scroll on Mobile) */}
             ...
             {shot.status === 'done' && shot.videoUrl && (
                 <div className="relative aspect-video bg-black border-t border-[var(--border-color)] group/video">
             ...
             {shot.status === 'processing' && (
                 <div className="absolute inset-0 bg-black/60 flex items-center justify-center z-20 backdrop-blur-[1px]">
                     <div className="text-center">
                         <i className="fas fa-circle-notch fa-spin text-2xl text-[var(--accent-cyan)] mb-2"></i>
                         <p className="text-xs font-bold text-white">{t('motionStudio.generating')}</p>
                     </div>
                 </div>
             )}
        </div>
...
const MotionStudio: React.FC<MotionStudioProps> = ({ theme, setTheme, isVip }) => {
    ...
    const [activeTab, setActiveTab] = useState<'text' | 'image' | 'character'>('text');
    ...
    return (
        <div className="flex-1 flex flex-col p-4 sm:p-6 md:p-8 font-sans animate-fade-in h-auto lg:h-full overflow-y-auto lg:overflow-hidden">
            ...
            {/* DYNAMIC GRID LAYOUT: Full width for Character Tab, Split for Text/Image */}
            <main className={`w-full max-w-7xl mx-auto grid gap-6 min-h-0 flex-1 lg:overflow-hidden ${isCharacterTab ? 'grid-cols-1' : 'grid-cols-1 lg:grid-cols-[1fr_350px]'}`}>
                
                <div className="flex flex-col gap-6 min-h-0 overflow-visible lg:overflow-hidden min-w-0">
                    <div className="flex bg-[var(--bg-component)] p-1 rounded-xl border border-[var(--border-color)] flex-shrink-0 shadow-sm">
                        <button onClick={() => setActiveTab('text')} className={`flex-1 py-3 rounded-lg font-bold text-sm flex items-center justify-center gap-2 transition-all ${activeTab === 'text' ? 'bg-[var(--bg-interactive)] text-[var(--accent-cyan)] shadow-sm' : 'text-[var(--text-secondary)] hover:text-white'}`}>
                            <i className="fas fa-font"></i> {t('motionStudio.tabs.text')}
                        </button>
                        <button onClick={() => setActiveTab('image')} className={`flex-1 py-3 rounded-lg font-bold text-sm flex items-center justify-center gap-2 transition-all ${activeTab === 'image' ? 'bg-[var(--bg-interactive)] text-[var(--accent-cyan)] shadow-sm' : 'text-[var(--text-secondary)] hover:text-white'}`}>
                            <i className="fas fa-image"></i> {t('motionStudio.tabs.image')}
                        </button>
                         <button onClick={() => setActiveTab('character')} className={`flex-1 py-3 rounded-lg font-bold text-sm flex items-center justify-center gap-2 transition-all ${activeTab === 'character' ? 'bg-[var(--bg-interactive)] text-[var(--accent-cyan)] shadow-sm' : 'text-[var(--text-secondary)] hover:text-white'}`}>
                            <i className="fas fa-users-cog"></i> {t('motionStudio.character.title')}
                        </button>
                    </div>

                    <div className={`bg-[var(--bg-component)] rounded-xl border border-[var(--border-color)] shadow-lg p-5 flex-shrink-0 transition-all duration-300 flex flex-col ${activeTab === 'character' ? 'flex-1 min-h-0 overflow-hidden' : 'min-h-[220px]'}`}>
                    ...
                    {/* Only show Shot List if NOT in character tab */}
                    {activeTab !== 'character' && (
                ...
                {/* Right Column: Controls (HIDDEN IF CHARACTER TAB) */}
                {!isCharacterTab && (
