...
const VideoAnalysisTab: React.FC<{
    ...
    // NEW: State to store user-uploaded replacement images for specific scenes
    // Key: scene.id, Value: base64 string of new image
    const [replacements, setReplacements] = useState<Record<string, string>>({});
    const fileInputRefs = useRef<Record<string, HTMLInputElement | null>>({});

    // Update internal state if props change (unlikely unless reset from parent)
    useEffect(() => {
    ...
    const handleVideoUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        setScenes([]);
        setReplacements({});
        onSaveScenes([]); // Clear parent state
        ...
        try {
            ...
            setScenes(newScenes);
            onSaveScenes(newScenes); // Persist to parent
        } catch (error) {
            console.error(error);
    ...
    // NEW: Handle replacing the image for a specific scene
        ...
        reader.onload = (e) => {
            if (e.target?.result) {
                setReplacements(prev => ({
                    ...prev,
                    [sceneId]: e.target!.result as string
                }));
            }
        };
        ...
        return (
            <div className="flex flex-col h-full">
                <div className="flex justify-between items-center mb-4">
                     <h3 className="font-bold text-[var(--accent-cyan)] uppercase tracking-wide">
                        {t('motionStudio.analysis.resultTitle')}
                    </h3>
                    <div className="flex gap-2">
                        <label htmlFor="video-upload-analysis-retry" className="cursor-pointer text-xs bg-[var(--bg-tertiary)] hover:bg-[var(--bg-hover)] px-3 py-1.5 rounded-lg border border-white/10 text-[var(--text-secondary)] transition-colors">
                            <i className="fas fa-redo mr-1"></i> {t('common.retry')}
                        </label>
                        <input type="file" accept="video/mp4,video/mov" onChange={handleVideoUpload} className="hidden" id="video-upload-analysis-retry" />
                        
                        <button onClick={handleClear} className="text-xs bg-red-500/10 hover:bg-red-500/20 px-3 py-1.5 rounded-lg text-red-400 border border-red-500/30 transition-colors">
                            <i className="fas fa-trash mr-1"></i> {t('motionStudio.clear')}
                        </button>
                    </div>
                </div>
                                    ...
                                    <div key={scene.id} className="bg-[var(--bg-deep-space)] border border-[var(--border-color)] rounded-xl overflow-hidden group hover:border-[var(--accent-cyan)] transition-all">
                                        <div className="aspect-video relative group/image">
                                            ...
                                            <div className="absolute top-2 left-2 bg-black/60 px-2 py-1 rounded text-[10px] text-white font-mono">
                                                {scene.timestamp} {isReplaced && <span className="text-yellow-400 ml-1">(Edited)</span>}
                                            </div>
                                            ...
                                            <div className="absolute inset-0 bg-black/50 opacity-0 group-hover/image:opacity-100 transition-opacity flex items-center justify-center">
                                                <button 
                                                    ...
                                                    className="bg-white/20 hover:bg-white/30 text-white font-bold py-1.5 px-3 rounded-full text-xs backdrop-blur-sm border border-white/20 flex items-center gap-1"
                                                >
                                                    <i className="fas fa-user-edit"></i> {t('motionStudio.analysis.changeActor')}
                                                </button>
                                                <input 
                                                    type="file" 
                                                    accept="image/*" 
                                                    ...
                                                    onChange={(e) => {
                                                        if (e.target.files?.[0]) handleReplaceImage(scene.id, e.target.files[0]);
                                                    }}
                                                />
                                            </div>
                                        </div>
                                        <div className="p-3">
                                            ...
                                            <div className="flex gap-2">
                                                ...
                                                <button 
                                                onClick={() => onUseShot(scene.prompt, currentImage)}
                ...
                <div className="flex flex-col gap-6 min-h-0 overflow-visible lg:overflow-hidden min-w-0">
                    ...
                    <div className={`bg-[var(--bg-component)] rounded-xl border border-[var(--border-color)] shadow-lg p-5 flex-shrink-0 transition-all duration-300 flex flex-col ${isFullWidth ? 'flex-1 min-h-0 overflow-hidden' : 'min-h-[220px]'}`}>
                        ...
                        {activeTab === 'character' ? (
                            ...
                            <VideoAnalysisTab 
                                onUseShot={handleUseAnalyzedShot} 
                                savedScenes={analyzedScenes} 
                                onSaveScenes={setAnalyzedScenes} 
                            />
                        ) : activeTab === 'text' ? (
                            <>
